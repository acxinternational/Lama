...
def get_tokenize_function(tokenizer, _max_length):
    def tokenize_function(examples):
        ...
        tokenized_inputs = tokenizer(
            ...
            # Padding type is to pad to the longest sequence in the batch (other option is to a certain max length,
            # or no padding)
            padding=True,
        )
...
def load_model(target_training_config, load_base_model=False):
    ...
    if "model_name" not in target_training_config:
        model_name = model_load_path
    else:
        model_name = target_training_config["model_name"]

    return model, tokenizer, device, model_name


# Trainer class to include logging and history
class LocalTrainer(transformers.Trainer):
    def __init__(
            self,
            model,
            model_flops,
